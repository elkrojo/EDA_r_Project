...
When volatile acidity is broken down into categorical values, the decrease in quality per alcohol volume is quite evident. All medians move progressively lower. The variance appears quite constant across the groups with High levels showing the least variance between the 1st and 3rd quartiles.
...
The concentration of citric acid has a positive correlation with quality per alcohol volume when divided into categorical groupings. All medians move progressively higher. The variance across the groups is quite constant with Low levels showing the least variance in the interquartile range.
...
Volatile acidity levels show no clear pattern with fixed acidity directly but seem to impact quality negatively. The fixed acidity value grows in variance as citric acid levels increase, while volatile acidity decreases across the same plots.
...
Citric acid levels show some correlation with fixed acidity with higher levels measured as fixed acidity increases. This correlation corresponds with Medium and High levels of volatile acidity where a similar pattern can be observed.
...
### Plot Two
```{r echo=FALSE, message=FALSE, warning=FALSE, Plot_Two_i}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Plot_Two_ii}
# wine quality by volatile acidity
...
Wines with the highest levels of volatile acidity bring the median quality score down. The highest scoring wines have Medium to High volatile acidity. The majority of the lowest scoring wines have Excessive levels of volatile acidity.
...
This plot highlights the relationship between alcohol volume and higher average scores. Separating out the volatile acidity levels shows how Excessive levels in particular drag the average score down while not showing much representation above 6 points of quality compared to lower levels.
...
The relationship between quality score and alcohol volume became evident early on. I was a little surprised that sulfur dioxide levels had a negligible impact on quality.
...
The combination of the other physiochemical elements in the wine does not appear to make any noticeable difference the the probability of receiving a higher score. Wine appears to be all about getting a good balance rather than an exact proportion.
